...
public class Program
{
    ...
    private final Properties checkProperties = new Properties();
    ...
    private static final File CHECK_PROPERTIES_FILE = new File("gcm4ml-checks.properties");
    ...
    protected void header(final String header)
    {
        if (System.console() == null)
        {
            throw new IllegalStateException("There's no console available!");
        }
        if (CHECK_PROPERTIES_FILE.isFile())
        {
            FileReader fr = null;
            try
            {
                fr = new FileReader(CHECK_PROPERTIES_FILE);
                checkProperties.load(fr);
            }
    ...
    protected String promptForTfsServer() throws IOException
    {
        final String TFS_SERVER_PROPERTY = "tfsServer";
        String tfsServer = checkProperties.getProperty(TFS_SERVER_PROPERTY);
        if (tfsServer != null)
        {
            final String template = "Using %1$s value '%2$s' from '%3$s'.";
            standardOut.println(String.format(template, TFS_SERVER_PROPERTY, tfsServer, CHECK_PROPERTIES_FILE.getAbsolutePath()));
        }
        else
        {
            standardOut.println();
            ...
            tfsServer = System.console().readLine();
            checkProperties.setProperty(TFS_SERVER_PROPERTY, tfsServer);
            saveCheckProperties();
        }
        return tfsServer;
    }

    protected void saveCheckProperties()
    {
        FileWriter fw = null;
        try
        {
            fw = new FileWriter(CHECK_PROPERTIES_FILE);
            checkProperties.store(fw, "Generated by " + getTitle() + " " + getVersion());
        }
        catch (final IOException e)
        {
            throw new Error(e);
        ...
        {
            IOHelper.closeQuietly(fw);
        }
    }

    protected String promptForUserName() throws IOException
    {
        final String USER_NAME_PROPERTY = "userName";
        String userName = checkProperties.getProperty(USER_NAME_PROPERTY);
        if (userName != null)
        {
            final String template = "Using %1$s value '%2$s' from '%3$s'.";
            standardOut.println(String.format(template, USER_NAME_PROPERTY, userName, CHECK_PROPERTIES_FILE.getAbsolutePath()));
        }
        else
        {
            userName = System.console().readLine("Please enter your TFS user name: ");
            checkProperties.setProperty(USER_NAME_PROPERTY, userName);
            saveCheckProperties();
        }
        return userName;
    }
    ...
    {
        final char[] passwordChars = System.console().readPassword("Please enter your TFS password: ");
        final String result = new String(passwordChars);
        return result;
    }
    ...
    private final Callable<Void> CheckTfs = new Callable<Void>()
    {
        ...
        {
            header("Running in 'CheckTfs' mode to query a TFS server for details.");
            final String repoUriString = promptForTfsServer();
    ...
    private final Callable<Void> CheckCredentials = new Callable<Void>()
    {
        ...
        public Void call() throws Exception
        {
            header("Running in 'CheckCredentials' mode to attempt authentication with the JVM's HTTP client.");
            final String repoUriString = promptForTfsServer();

            final String userName = promptForUserName();
            final String password = promptForPassword();
